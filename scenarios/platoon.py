...
class Platoon():
    ...
    def __init__(self, startingVehicles):
        ...
        self._targetSpeed = -1
        self._collisionDetected = False
        self._allocatedSpeed = 0

        self.getLeadVehicle().setColor((255, 0, 0))
    ...
    def setCollisionDetected(self,collisionDetected):
        self._collisionDetected = collisionDetected
    
    def getCollisionDetected(self):
        return self._collisionDetected

    def setAllocatedSpeed(self,allocatedSpeed):
        self._allocatedSpeed = allocatedSpeed

    def getAllocatedSpeed(self):
        return self._allocatedSpeed
    ...
    def getLength(self):
        ...
        #print("lead Id"+self.getLeadVehicle().getName())
        #print("rear Id"+self._vehicles[-1].getName())
        #print("This is total length of platoon#######" + str(rear - front + rearVehicleLength))
    ...
    def _updateSpeed(self, speed, inclLeadingVeh=True):
        ...
        """
        if self._collisionDetected:
            self.getLeadVehicle().setSpeed(self._allocatedSpeed)
        else:
            if inclLeadingVeh and self.getLeadVehicle().getLane() not in self._controlledLanes:
                #print("Lead Veh ID and Speed"+str(self.getLeadVehicle().getName())+str(speed))
                self.getLeadVehicle().setSpeed(speed)
                #print("lane position" + str(self.getLanePositionFromFront()))

            leadVehEdge = self.getLeadVehicle().getEdge()
            targetLane = self.getLeadVehicle().getLaneIndex()

            # If lead vehicle is more than 60m from intersection and if it has followers slow down 
            # so that follower can catch up
            #if self.getLanePositionFromFront() > 50:
                #self.getLeadVehicle().setSpeed(6)
            ...
            for veh in vehicles:
                ...
                if veh.getLane() not in self._controlledLanes:
                    ...
                    if leadVeh and leadVeh[1] <= 5 and self._currentSpeed != 0:
                        #print("set veh speed"+str(veh.getName())+"speed"+str(speed)+"lane"+str(veh.getLane())+"lead"+str(self.getLeadVehicle().getName())+"leadSpeed"+str(self.getLeadVehicle().getSpeed()))
                        veh.setSpeed(speed)
                    else:
                        veh.setSpeed(-1)
