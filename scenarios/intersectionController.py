...
class IntersectionController():
    ...
    def calculateNewReservedTime(self, pv, reservedTime):
        ...
        if reservedTime == 0:
            ...
            #print("lenThruJunc"+str(lenThruJunc))
        else:
            lenThruJunc = pv.getLength() * 2 if self.zip else 1
           # print("lenThruJunc"+str(lenThruJunc))
            #print("pv speed"+str(pv.getSpeed()))
        resT = (0.5 if self.zip else 1.5) + reservedTime + (lenThruJunc / (pv.getSpeed() or 1))
        #print("Res"+str(resT))
    ...
    def getVehicleZipOrderThroughJunc(self):
        ...
        if self.zip:
            prelimList = simlib.flatten([self._zipPlatoons(z) for z in self.platoonZips])
    ...
    def getNewSpeed(self, pv, reservedTime):
        ...
        # If we are in the last 10 metres, we assume no more vehicles will join the platoon
        ...
        if self.zip:
            ...
            for p in self.platoons:
                ...
                if p.getLane() in self.lanesServed:
                    ...
                    #print("IntersectionController Platoon"+ p.getLeadVehicle().getName()+"Speed"+str(speed)+"ReseTi"+str(reservedTime))
                    reservedTime = self.calculateNewReservedTime(p, reservedTime)
                    #print("Calculated Reserve Time"+ str(reservedTime))
